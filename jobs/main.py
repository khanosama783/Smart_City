...
# latitude and longitude of landon and birmingham
...
def emergency_incident_data(device_id, timestamp, location):
    return {
        "id": uuid.uuid4(),
        "deviceId": device_id,
...
def delivary_report(err, msg):
    if err is not None:
        print(f"Message delivery to {msg.topic() [msg.partition()]}")
    else:
        print(f"Messge delivered to {msg.topic()} [{msg.partition()}]")
...
def simulate_journey(producer, device_id):
    while True:
        ...
        traffic_camera_data = generate_traffic_camera_data(
            device_id, vehicle_data["timestamp"], "camera_id"
        )
        weather_data = generate_weather_data(
            device_id,
            vehicle_data["timestamp"],
            (vehicle_data["location"][0], vehicle_data["location"][1]),
        )
        _emergency_incident_data = emergency_incident_data(
            device_id,
            vehicle_data["timestamp"],
            (vehicle_data["location"][0], vehicle_data["location"][1]),
        )
        ...
        producer_data_to_kafka(producer, EMERGENCY_TOPIC, _emergency_incident_data)
...
if __name__ == "__main__":
    ...
    try:
        simulate_journey(producer, "Vehicle-777")
